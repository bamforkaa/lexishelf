...
- 상태: Accepted
- 날짜: 2026-08-23 (2026-08-26 Task 11 범위 확장)
...
한국어기초사전은 11개 외국어를 지원하지만 German, Hindi, Polish, Latin, Dutch, Portuguese, Italian, Turkish, Czech, Swedish, Finnish, Ukrainian은 포함하지 않는다. PanLex는 많은 language variety의 expression을 source별 meaning/denotation 관계로 제공하며 local-first fallback 후보가 될 수 있다.
...
- 현재 범위는 실제 coverage를 확인한 `deu-000`, `hin-000`, `pol-000`, `lat-000`, `nld-000`, `por-000`, `ita-000`, `tur-000`, `ces-000`, `swe-000`, `fin-000`, `ukr-000`과 `kor-000`의 양방향 exact translation이다.
...
- 최종 189,714,432-byte SQLite index는 user Room과 분리하고 release별 app private directory에서 read-only로 조회한다.
- 체크섬으로 고정한 2019-09-01 artifact의 embedded CC0 grant 아래 import mode는 `COPY_EXPORTABLE_FIELDS`이지만 row toggle로 선택된 번역과 provenance만 user vocabulary sense로 복사한다. 현재 공식 PanLex license는 다르므로 새 artifact에 이 정책을 자동 적용하지 않는다. dataset 전체를 Room/backup에 저장하지 않는다.
...
- 12개 외국어에서 network/API key 없이 한국어 양방향 exact fallback을 제공한다.
...
- 190MB raw SQLite/72.5MB compressed pack이 debug APK 크기와 첫 install 시간을 늘린다. 추가 확대 전 language-group sub-pack이 필요한지 다시 측정해야 한다.
...
- 한국어기초사전의 기존 11개 언어까지 한 번에 포함: 현재 provider와 중복되고 초기 pack 비용만 늘린다.
