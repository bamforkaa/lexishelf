...
canonical backup은 사용자가 소유한 단어장 데이터만 담는 UTF-8 JSON입니다. 단어, BCP 47 언어 태그, 순서가 있는 뜻/품사/예문, 메모, 태그와 다대다 관계, 생성·수정 시간을 포함합니다. 현재 domain model에는 favorite와 review metadata가 없으므로 schema v1에는 존재하지 않습니다.
...
Room schema version과 backup schema version은 독립적입니다. 현재 Room은 version 2이고 JSON은 `schemaVersion: 1`입니다.
...
필수 필드는 위 예시의 모든 필드입니다. 빈 DB는 빈 `tags`와 `entries` 배열로 표현합니다. JSON의 unknown field도 현재는 거부하여 잘못 이해한 문서를 조용히 적용하지 않습니다.
...
3. `schemaVersion`을 먼저 확인합니다. 현재는 1만 지원합니다.
4. version 1 DTO 전체를 strict decoding하고 필수/unknown field를 확인합니다.
5. format, stable ID, 중복 ID/태그 이름, BCP 47 태그, 필수 text, timestamps, sense/example, tag reference를 검증하고 canonicalize합니다.
...
1. 새 `VocabularyBackupV2` DTO와 validation을 추가합니다.
...
Room schema가 3 이상으로 바뀌더라도 JSON이 표현하는 사용자 데이터의 의미가 그대로라면 backup schema는 올릴 필요가 없습니다. 반대로 DB 변경 없이도 JSON 계약이 바뀌면 backup schema만 올릴 수 있습니다.
