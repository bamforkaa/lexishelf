...
`bundleDictionaryPacksInDebug`는 개발자 Manual QA 전용 opt-in입니다. `true`이면 debug APK가 커지지만 `assembleDebug`와 `installDebug`가 먼저 pack 네 개를 재생성하여 APK에 포함합니다. 첫 앱 실행은 `AndroidDictionaryPackRepository`의 production manifest/크기/SHA-256/payload 검증과 atomic activation을 그대로 사용합니다. release APK에는 적용되지 않습니다.
...
1. 기존 `cc-cedict`, `korean-basic`, `panlex`, `jmdict` directory를 새 root 아래로 동일한 layout을 유지해 직접 이동합니다.
...
└─ kaikki/{source,generated,packs}/   # future convention only
```

현재 네 provider의 정확한 개발 파일은 다음과 같습니다. CC-CEDICT는 SQLite 변환 단계가 없고 GZip source가 그대로 payload가 됩니다.
...
`build_jmdict_index.py`, `build_krdict_index.py`, `build_panlex_index.py`는 input/output을 명시할 수도 있습니다. 둘 다 생략하면 위 canonical convention을 사용합니다. CC-CEDICT는 별도 변환 없이 source GZip을 pack payload로 사용합니다. pack builder는 repository의 legacy Android assets를 fallback 입력으로 사용하지 않습니다.
...
전환 전 legacy ignored asset 위치도 한시적으로 입력 후보로 읽지만 base APK asset source set에는 포함하지 않습니다. 생성된 `.dictpack`은 각 dataset의 `packs/`에 생깁니다.
...
네 pack을 명시적으로 이름을 확인한 Manual AVD의 Downloads에 일괄 staging하려면 다음 helper를 사용합니다.

```powershell
python -m tools.stage_dictionary_packs --avd-name Medium_Phone_Manual
```

이 helper는 Test AVD나 이름이 다른 기기로 자동 fallback하지 않으며 pack을 활성화하지도 않습니다. `bundleDictionaryPacksInDebug=false`일 때는 staging 후 앱의 설정 → `로컬 pack 설치`에서 각 파일을 선택해야 합니다. Manual QA에서 옵션을 `true`로 설정하면 별도 staging 없이 `installDebug` 후 첫 실행에 네 pack이 검증·활성화됩니다. 같은 payload hash의 활성 pack은 건너뜁니다.
...
`manifestSchemaVersion`은 pack envelope version이고 provider dataset SQLite의 `datasetSchemaVersion`, user Room schema v4, JSON backup schema v3와 서로 독립적입니다.
...
| `panlex.ko-fallback` | `2019-09-01` | 53,211,136 | `0f40e4cc6174602196a04bd4ac9d38893a01a0a075692d746f7e6f40b0995712` | 21,044,212 |
...
Task 8의 dataset 포함 debug APK는 144,462,083 bytes였습니다. Task 9의 pack 미포함 base APK는 18,466,693 bytes입니다. Manual QA opt-in으로 pack 네 개를 포함한 2026-08-25 debug APK는 129,043,442 bytes이며, release/base APK 분리 정책에는 영향을 주지 않습니다.
