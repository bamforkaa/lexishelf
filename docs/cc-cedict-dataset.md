...
4. 파일을 다음 경로에 그대로 복사합니다. 압축을 풀거나 user vocabulary Room DB로 import하지 않습니다.

```text
app/src/main/assets/dictionary/cccedict/cedict_1_0_ts_utf-8_mdbg.txt.gz
```

PowerShell 예시는 사용자가 내려받은 정확한 파일을 지정해야 합니다.

```powershell
New-Item -ItemType Directory -Force .\app\src\main\assets\dictionary\cccedict
Copy-Item -LiteralPath 'C:\path\chosen-by-user\cedict_1_0_ts_utf-8_mdbg.txt.gz' `
  -Destination .\app\src\main\assets\dictionary\cccedict\cedict_1_0_ts_utf-8_mdbg.txt.gz
...
APK에는 수동으로 둔 GZip 원본이 asset으로 들어갑니다. 첫 CC-CEDICT 검색에서만 background I/O dispatcher가 UTF-8 text를 parse하고 Simplified/Traditional exact-match map을 메모리에 만듭니다. 이후 같은 app process의 검색은 해당 index를 재사용합니다. user vocabulary Room schema와 별도이며 앱 시작이나 첫 화면에서는 import 작업을 하지 않습니다.
...
6. 기존 asset을 새 파일로 교체해 `testDebugUnitTest`, `lintDebug`, `assembleDebug`를 실행하고 실제 APK 크기/첫 검색 비용을 기록합니다.
7. update 실패 시 이전에 검증한 GZip을 되돌립니다. user Room DB에는 어떤 변경도 하지 않습니다.

향후 language pack으로 전환할 때는 `CcCedictDatasetSource`의 asset 구현을 설치된 pack reader로 교체합니다. parser, index, provider mapping, registry와 사용자 DB는 그대로 유지할 수 있습니다.
